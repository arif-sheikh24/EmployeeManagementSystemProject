...
if($_SERVER['REQUEST_METHOD'] !='POST'){
    echo "<script> alert('Error: No data to save.'); location.replace('./index.php?page=calender') </script>";
    $conn->close();
    exit;
}
extract($_POST);
...
$save = $conn->query($sql);
if($save){
    echo "<script> alert('Schedule Successfully Saved.'); location.replace('./index.php?page=calender') </script>";
}else{
    echo "<pre>";
