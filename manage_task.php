...
<div class="form-group">
	<form action="" id="manage-task">
		...
		<div class="form-group">
            ...
            <div class="form-group">
              <label for="" class="control-label">Project assign task</label>
              <select class="form-control form-control-sm select2" multiple="multiple" name="username">
              	<option></option>
              	<?php 
              	$employees = $conn->query("SELECT *,concat(username) as name FROM users where type = 3 order by concat(username) asc ");
              	while($row= $employees->fetch_assoc()):
              	?>
              	<option value="<?php echo $row['username'] ?>" <?php echo isset($username) && in_array($row['username'],explode(',',$username)) ? "selected" : '' ?>><?php echo ucwords($row['username']) ?></option>
              	<?php endwhile; ?>
              </select>
            </div>
